...
class ReportService {
    // static ReportEvents = async ({ body, type }: { body: GetReport, type?: TypeReport }): Promise<useReportProps> => {
    // const response = await useAppStore.getState().instance.post(`reports/${type}`, body);

    // const { data: dataResponse, ...rest } = response;
    // const data = dataResponse as { nombre: string, cuentas?: Account[], fechas?: string[], total?: number, percentajes?: Percentajes };
