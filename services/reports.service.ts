import { Alarm, AP, APCI, Bat, BatteryStatus, CI, GetReport, otros, Percentajes, Prue, ResponseReport, TypeReport } from "@/interface/hooks.interface";
import useAppStore from "@/utils/app.store";

class ReportService {
    static ReportEvents = async ({ body, type }: { body: GetReport; type?: TypeReport; }) => {
        const response = await useAppStore.getState().instance.post<ResponseReport>(`reports/${type}`, body);
        const { data } = response;
        if (data.cuentas?.length === 1 && data.cuentas[0].eventos) {
            const total: number = data.cuentas[0].eventos.length;
            if (type === 'ap-ci') {
                let Aperturas = data.cuentas[0].eventos.filter(f =>
                    AP.find(ff => ff === f.CodigoAlarma),
                ).length;
                let Cierres = data.cuentas[0].eventos.filter(f =>
                ...
                let ApCi = data.cuentas[0].eventos.filter(f =>
                    APCI.find(ff => ff === f.CodigoAlarma),
                ).length;
                let Alarma = data.cuentas[0].eventos.filter(f =>
                    Alarm.find(ff => ff === f.CodigoAlarma),
                ).length;
                let Pruebas = data.cuentas[0].eventos.filter(f =>
                    Prue.find(ff => ff === f.CodigoAlarma),
                ).length;
                let Bate = data.cuentas[0].eventos.filter(f =>
                    Bat.find(ff => ff === f.CodigoAlarma),
                ).length;
                let Otros = data.cuentas[0].eventos.filter(f =>
            ...
            if (data && data.cuentas && data.total) {
                let conRestaure: number = 0,
                    sinRestaure: number = 0,
                    sinEventos: number = 0;
                sinRestaure = data.cuentas.filter(
                    acc => acc.estado === BatteryStatus.ERROR,
                ).length;
                conRestaure = data.cuentas.filter(
                    acc => acc.estado === BatteryStatus.RESTORE,
                ).length;
                sinEventos = data.cuentas.filter(
                    acc => acc.estado === BatteryStatus.WITHOUT_EVENTS,
                ).length;
            ...
            if (data && data.cuentas) {
                let abiertas: number = 0,
                    cerradas: number = 0,
                    sinEstado: number = 0;
                abiertas = data.cuentas.filter(f =>
                    AP.find(ff => ff === f.evento?.CodigoAlarma),
                ).length;
                cerradas = data.cuentas.filter(f =>
                    CI.find(ff => ff === f.evento?.CodigoAlarma),
                ).length;
            ...
            if (data && data.cuentas) {
                let reciberAp: number = 0,
                    reciberCi: number = 0;
                const acc = data.cuentas.map(acc => {
                    if (acc.eventos && data.fechas) {
                        const { eventos, ...rest } = acc;
                        const df = data.fechas
                            .map(day => {
                                const perDay = acc.eventos?.filter(
                                    ev => ev.FechaOriginal === day,
                                );
                                if (perDay && perDay.length > 0) {
                                    let Aperturas = perDay
                                        .filter(f => AP.find(ff => ff === f.CodigoAlarma))
                                        .slice(0, 1);
                                    let Cierres = perDay
                                        .filter(f => CI.find(ff => ff === f.CodigoAlarma))
                                        .reverse()
                                        .slice(0, 1);
